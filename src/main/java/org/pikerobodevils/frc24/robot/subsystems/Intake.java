...
public class Intake extends SubsystemBase {
  ...
  private final CANSparkMax intakeMotor =
      new CANSparkMax(MOTOR_ID, CANSparkLowLevel.MotorType.kBrushless);
  ...
  public Command stopInake() {
    ...
    return run(() -> setSpeed(0))
        .andThen(
            startEnd(
                    () -> controlboard.driver.getHID().setRumble(RumbleType.kBothRumble, .5),
                    () -> controlboard.driver.getHID().setRumble(RumbleType.kBothRumble, 0))
                .withTimeout(.5));
  }
  ...
  public Command stopOuttake() {
    ...
    return startEnd(
            () -> controlboard.operator.getHID().setRumble(RumbleType.kBothRumble, .5),
            () -> controlboard.operator.getHID().setRumble(RumbleType.kBothRumble, 0))
        .withTimeout(.5);
  }
