...
public class REALArm implements ArmIO {
  ...
  SparkMax leftController = new SparkMax(LEFT_CONTROLLER_ID, CANSparkLowLevel.MotorType.kBrushless);
  SparkMax rightController =
      new SparkMax(RIGHT_CONTROLLER_ID, CANSparkLowLevel.MotorType.kBrushless);

  // Encoder encoder = new Encoder(ENCODER_QUAD_A, ENCODER_QUAD_B, true,
  ...
  public void updatePositionController() {
    ...
    var feedforwardOutput =
        feedforward.calculate(
            MathUtil.angleModulus(absoluteEncoder.getDistance()), setpoint.velocity);
